...
class Solution:
    def compareVersion(self, version1: str, version2: str) -> int:
        vl1 = list(map(int, version1.split('.')))
        vl2 = list(map(int, version2.split('.')))

        for i in range(max(len(vl1), len(vl2))):
            v1 = vl1[i] if i < len(vl1) else 0
            v2 = vl2[i] if i < len(vl2) else 0
            if v1 < v2:
                return -1
            elif v1 > v2:
                return 1
